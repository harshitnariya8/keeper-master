...
def custDashboard(request):
    ...
    return render(request, 'customer/dashboard.html',
                  {'customer_data': customer_data,
                   'near_by_sellers': sorted_nn,
                   'customer_add': ref_location,
                   'cus_add': cus_add,
...
def checkout(request):
    if request.method == "POST":
        ...
        amount =( int(request.POST.get('total').replace('/', '')) / 100 )+100
        list_of_orders = request.POST.get('list_of_orders').replace('/', '')

        client = razorpay.Client(auth=("rzp_test_yEk1OQFDBGt7Yx", "sz7eV2QdINrzZvmkhPBRfHwV"))
